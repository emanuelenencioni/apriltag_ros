...
AprilTagNode::AprilTagNode(const rclcpp::NodeOptions& options)
    ...
    this->parent_frame_id = declare_parameter("parent_frame_id", std::string("camera"), descr("parent frame of the published tag poses", true));
...
void AprilTagNode::onCamera(const sensor_msgs::msg::Image::ConstSharedPtr& msg_img,
    ...
    for (int i = 0; i < zarray_size(detections); i++) {
        ...
        if (estimate_pose != nullptr && calibrated) {
            ...
            tf.header.frame_id = this->parent_frame_id;  // set parent frame from parameter
